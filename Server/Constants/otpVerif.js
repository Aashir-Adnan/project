...
async function isValidAccessToken(res, accessToken, decryptedBody) {
    ...
    try {
        ...
        const result = await executeQuery(query, [accessToken, decryptedBody.email, decryptedBody.device_name], connection);
        if (result.length > 0) {
...
async function verifyOTP(res, OTP, decryptedBody, updatedFlag = 1) {
    ...
    const otpResult = await executeQuery(otpQuery, [userId, OTP, device_name], connection);
    ...
    `;
    connection = await projectDB();
    user_role = await executeQuery(userRoleDataQuery, [userId], connection)
