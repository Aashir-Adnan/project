...
export default function SortData({
  ...
  const handleServerDrivenSortToggle = (name) => {
    console.log("handleServerDrivenSortToggle",name,permission);
    if (!permission) {
      onAction("Permission denied: sorting is disabled.");
      return;
    }
  ...
  return (
    <div>
      ...
      >
        {dataHeaders
          ?.filter((element) => !shouldExclude(element?.dynamicKey || element?.name ))
          .map((element, index) => {
            ...
            return (
              <MenuItem
                key={index}
                onClick={() => isServerDriven?  handleServerDrivenSortToggle(element?.dynamicKey || element?.name):handleSortToggle(element?.dynamicKey || element?.name)}
              ...
              >
                ...
                >
                  <span
                    style={{ minWidth: longestHeaderWidth || 200, textAlign: "left" }}
                  >
                    {element.label}
                  </span>
                  {activeSort ? (
