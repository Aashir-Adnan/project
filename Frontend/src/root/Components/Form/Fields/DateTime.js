...
serverMode,
  ...
  const minDateTime = field.minDateTime === "today" ? today : field.minDateTime;
  const maxDateTime = field.maxDateTime === "today" ? today : field.maxDateTime;
  if (field.hidden){
        ...
        value={
          Array.isArray(formValues)
            ? formValues[currentStep]?.[field?.dynamicKey]
              ? new Date(formValues[currentStep][field?.dynamicKey]).toISOString().slice(0, 16)
              : ""
            : formValues[field?.dynamicKey]
            ? new Date(formValues[field?.dynamicKey]).toISOString().slice(0, 16)
            : ""
        }
        variant={variant}
